...
If (isset($_POST['knpToon']) && !empty($_POST['kzljaar'])) { $jaar = $_POST['kzljaar'];	unset($label); }
Include "header.php"; ?>
...
WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and date_format(h.datum,'%Y') >= '$jaarstart' and h.actId = 1 and v.mdrId is not null
...
FROM (
	SELECT date_format(h.datum,'%Y%m') jrmnd, Month(h.datum) maand, year(h.datum) jaar, count(distinct v.mdrId) worpat, count(h.hisId) gebaant, 
	 round((count(h.hisId) / count(distinct v.mdrId)),2) gemworp, count(s.levensnummer) levnrat, count(hs.hisId) speenat
	FROM tblHistorie h
	 join tblStal st on (st.stalId = h.stalId)
	 join tblSchaap s on (s.schaapId = st.schaapId)
	 join tblVolwas v on (v.volwId = s.volwId)
	 left join tblHistorie hs on (st.stalId = hs.stalId and hs.actId = 4)
	WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and h.actId = 1 and year(h.datum) = '".mysqli_real_escape_string($db,$jaar)."' and v.mdrId is not null
	GROUP BY Month(h.datum), year(h.datum)
 ) aant
 left join (
	SELECT date_format(h.datum,'%Y%m') jrmnd, Month(h.datum) maand, Year(h.datum) jaar, count(s.schaapId) doodgeb
	FROM tblSchaap s
	 join tblVolwas v on (v.volwId = s.volwId)
	 join tblStal st on (s.schaapId = st.schaapId)
	 join tblHistorie h on (st.stalId = h.stalId)
	 join tblHistorie ho on (st.stalId = ho.stalId and ho.actId = 14)
	WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and h.actId = 1 and h.datum = ho.datum and (isnull(s.momId) or s.momId = 1) 
	 and year(h.datum) = '".mysqli_real_escape_string($db,$jaar)."' and v.mdrId is not null
	GROUP BY month(h.datum), Year(h.datum)
 ) dgeb on (aant.jrmnd = dgeb.jrmnd)
 left join (
	SELECT date_format(h.datum,'%Y%m') jrmnd, Month(h.datum) maand, Year(h.datum) jaar, count(s.schaapId) onvdoodgeb
		FROM tblSchaap s
		 join tblVolwas v on (v.volwId = s.volwId)
		 join tblStal st on (s.schaapId = st.schaapId)
		 join tblHistorie h on (st.stalId = h.stalId)
		 join tblHistorie ho on (st.stalId = ho.stalId and ho.actId = 14)
	WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and h.actId = 1 and h.datum = ho.datum and s.momId = 2 and year(h.datum) = '".mysqli_real_escape_string($db,$jaar)."' and v.mdrId is not null
	GROUP BY month(h.datum), Year(h.datum)
 ) odgeb on (aant.jrmnd = odgeb.jrmnd)
 left join (
	SELECT date_format(h.datum,'%Y%m') jrmnd, Month(h.datum) maand, Year(h.datum) jaar, count(s.schaapId) vrmerk
		FROM tblSchaap s
		 join tblVolwas v on (v.volwId = s.volwId)
		 join tblStal st on (s.schaapId = st.schaapId)
		 join tblHistorie h on (st.stalId = h.stalId)
		 join tblHistorie ho on (st.stalId = ho.stalId and ho.actId = 14)
	WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and h.actId = 1 and h.datum < ho.datum and isnull(s.levensnummer) and year(h.datum) = '".mysqli_real_escape_string($db,$jaar)."' and v.mdrId is not null
	GROUP BY month(h.datum), Year(h.datum)
 ) merk on (aant.jrmnd = merk.jrmnd)
 left join (
	SELECT date_format(h.datum,'%Y%m') jrmnd, Month(h.datum) maand, Year(h.datum) jaar, count(s.schaapId) vrspeen
		FROM tblSchaap s
		 join tblVolwas v on (v.volwId = s.volwId)
		 join tblStal st on (s.schaapId = st.schaapId)
		 join tblHistorie h on (st.stalId = h.stalId)
		 join tblHistorie ho on (st.stalId = ho.stalId and ho.actId = 14)
		 left join tblHistorie hs on (st.stalId = hs.stalId and hs.actId = 4)
	WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and h.actId = 1 and isnull(hs.actId) and s.levensnummer is not null and year(h.datum) = '".mysqli_real_escape_string($db,$jaar)."' and v.mdrId is not null
	GROUP BY month(h.datum), Year(h.datum)
 ) do_spn on (aant.jrmnd = do_spn.jrmnd)
 left join (
	SELECT date_format(h.datum,'%Y%m') jrmnd, sum((hs.kg -  h.kg)*1000/ DATEDIFF(hs.datum, h.datum)) groeidag, count(st.schaapId), 
	round(sum((hs.kg -  h.kg)*1000/ DATEDIFF(hs.datum, h.datum)) / count(st.schaapId),2) gemgroeidag
	FROM tblSchaap s 
	 join tblVolwas v on (v.volwId = s.volwId)
	 join tblStal st on (st.schaapId = s.schaapId)
	 join tblHistorie h on (st.stalId = h.stalId and h.actId = 1)
	 join tblHistorie hs on (st.stalId = hs.stalId and hs.actId = 4)
	WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and year(h.datum) = '".mysqli_real_escape_string($db,$jaar)."' and v.mdrId is not null
	GROUP BY Month(h.datum), Year(h.datum) 
	...
	FROM (
		...
		FROM (
			...
			WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and a1.aan = 1 and a2.uit = 1 and h1.skip = 0 and h2.skip = 0
			 and p.doelId = 1 and year(h1.datum) = '".mysqli_real_escape_string($db,$jaar)."'
		 ...
		 join tblHistorie hgeb on (hgeb.stalId = st.stalId and hgeb.actId = 1)
...
<th style = "text-align:center;"valign= "bottom";width= 80><h3><?php echo $jaar; ?> </h3> Aantal worpen <hr></th>
...
<?php	while($row = mysqli_fetch_array($result))/*	$row zorgt voor de waardes per maand 	*/
		{ $mndnr = $row['maand'];
 ...
 (
	...
	FROM (
	 ...
	 join (
		...
		FROM tblStal st
		 join tblHistorie h on (st.stalId = h.stalId)
		WHERE h.actId = 1 and st.lidId = ".mysqli_real_escape_string($db,$lidId)." and date_format(h.datum,'%Y') = ".mysqli_real_escape_string($db,$jaar)." and Month(h.datum) = ".mysqli_real_escape_string($db,$mndnr)."
...
<?php	if(isset($totLevnr)) {$totLevnr = $totLevnr+$row['levnrat']; } else { $totLevnr = $row['levnrat']; } ?>
 <td width = 1> </td>
 <td width = 100 style = "font-size:15px;"> <?php echo $row['d_vrspeen']; ?> <br> </td>
...
<tr align = "center">	
 <td width = 0> </td>
 <td width = 250 style = "font-size:15px;"> <hr /><b> Totaal&nbsp<?php echo $jaar; ?> </b> </td>
 <td width = 1> </td>
...
FROM tblHistorie h
 ...
 join tblVolwas v on (v.volwId = s.volwId)
 left join tblHistorie hs on (st.stalId = hs.stalId and hs.actId = 4)
WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and h.actId = 1 and year(h.datum) = '".mysqli_real_escape_string($db,$jaar)."' and v.mdrId is not null and hs.hisId is not null
") or die (mysqli_error($db));
