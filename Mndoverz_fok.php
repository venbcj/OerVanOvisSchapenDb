...
/*if (isset($_GET['jaar'])) { $kzlJaar = $_GET['jaar']; }	else*/if (isset($_POST['kzlJaar'])) { $kzlJaar = $_POST['kzlJaar']; }
...
SELECT 15 maand, 14 jaar, 13 worpat, 12 gebaant, 11 gemworp, 10 levnrat, 9 speenat, 
 8 doodgeb, 7 perc_doodgeb, 
 6 onvdoodgeb, 5 perc_onvdood, 
 4 vrmerk, 3 perc_vrmerk,
 2 d_vrspeen, 1 perc_vrspeen,
 16 daggroei,
 17 voer
FROM (
	SELECT count(distinct v.mdrId) worpat, count(h.hisId) gebaant, 
	...
	FROM tblHistorie h
	 ...
	 left join (
		...
		WHERE h.actId = 4 and h.skip = 0 and st.lidId = '13'
	 ) spn on (spn.schaapId = s.schaapId)
	WHERE st.lidId = '13' and h.actId = 1 and h.skip = 0 and year(h.datum) = '2025' and v.mdrId is not null
