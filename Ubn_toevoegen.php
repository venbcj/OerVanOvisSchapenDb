...
// Zoek naar ubn op duplicaten
    ...
    if (empty($_POST['insUbn_']))                 {
        $fout = "U heeft geen ubn ingevoerd."; 
    }    else if ($dubbel)         {
        $fout = "Dit ubn bestaat al.";
        $new_ubn = '';
    }    else {
        $ubn_gateway->insert_with_plaats($lidId, $new_ubn, $new_adres, $new_plaats);
    }
} ?>
...
<?php
// START LOOP    
$zoek_ubn = $ubn_gateway->zoek_met_plaats($lidId);
    while($row = mysqli_fetch_assoc($zoek_ubn)) {
      ...
      $actief = $row['actief'];
      $dbRelatie = $ubn_gateway->zoek_relatie($row['ubnId']);
...
<tr>
 ...
 <td align="center"> 
    <input type="checkbox" name="chbActief_<?php echo $Id; ?>" id="c1" value="1"<?php echo $actief == 1 ? ' checked' : ''; ?>
title="Is ubn te gebruiken ja/nee ?">
 </td>
 <td align="center">
     <?php if(isset($dbRelatie)) { ?>
     <input type="checkbox" name="chbDel_<?php echo $Id; ?> >"
 <?php } ?>
 </td>
...
if($actief == 0) {
    echo $txtAdres;
     ...
     <input type="text" name="txtAdres_<?php echo $Id; ?>" value="<?php echo $txtAdres; ?>">
<?php } ?>
 </td>
 <td style="color: <?php echo $color; ?> ;" >
<?php
    if($actief == 0) {
     ...
     <input type="text" name="txtPlaats_<?php echo $Id; ?>" size="12" value="<?php echo $txtPlaats; ?>">
<?php } ?>
 </td>
</tr>
<?php    } ?>
