...
") or die(mysqli_error($db));
    ...
    foreach ($id as $key => $value) {
        if ($key == 'txtcreUbn' && !empty($value)) {
            $fldUbn = $value;
        }
        if ($key == 'txtcreNaam' && !empty($value)) {
            $fldNaam = $value;
        }
        if ($key == 'txtcrePres' && !empty($value)) {
            $fldPres = $value;
        }
        if ($key == 'txtcreStraat' && !empty($value)) {
            $fldStraat = $value;
        }
        if ($key == 'txtcreNr' && !empty($value)) {
            $fldNr = $value;
        }
        if ($key == 'txtcrePc' && !empty($value)) {
            $fldPc = $value;
        }
        if ($key == 'txtcrePlaats' && !empty($value)) {
            $fldPlaats = $value;
        }
        if ($key == 'txtcreTel' && !empty($value)) {
            $fldTel = $value;
        }
        if ($key == 'chkcreActief' && !empty($value)) {
            $fldActief = $value;
        }
    }
    if (!isset($fldPres) && isset($fldNaam)) {
        $fldPres = $fldNaam;
    }
    if (!isset($fldActief)) {
        $fldActief = 0;
...
";
    ...
    // geheel ADRES invoeren
        if (
            !isset($adrId_db) && ( isset($fldStraat) || isset($fldNr) || isset($fldPc) || isset($fldPlaats)
            )
        ) {
