...
$inhoud = file($lok); //Leest bestand uit
$c = count($inhoud); /*Aantal regels uit bestand */
for ($i = 0; $i < $c; $i++) {
    $regel = explode(";", $inhoud[$i]);
    $cc = 44; // Aantal velden per regel zijn bij BurgM 46 elementen !! dan gaat het fout als $cc variabel is.
    for ($ii = 0; $ii < $cc; $ii++) {
        if ($ii == 0) {
            $insert_qry = " INSERT INTO impReader SET " . $velden[$ii] . " = '" . $regel[$ii] . "'";
        } elseif ($regel[$ii] == "" || $regel[$ii] == "0") {
            $insert_qry .= ", " . $velden[$ii] . " = NULL";
        } else {
            $insert_qry .= ", " . $velden[$ii] . " = '" . $regel[$ii] . "'";
        }
    }
...
if ($reader_aanwezig == 1) {
    $DelFile = $dir . "/" . $stream;
    unlink($DelFile) or die("Kan bestand " . $inputfile . " niet verwijderen. " . mysqli_error($db));
}
