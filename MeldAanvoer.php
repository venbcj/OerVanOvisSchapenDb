...
") or die(mysqli_error($db));
    ...
    if (isset($dmaanw)) {
        if ($geslacht == 'ooi') {
            $fase = 'moederdier';
            // TODO: geen elseif, maar else--of hebben schapen nog meer geslachten?
        } elseif ($geslacht == 'ram') {
            $fase = 'vaderdier';
        }
    } else {
...
$naam = $p['naam'];
 ...
 <!--hidden checkbox, zelfde name: uitgevinkt wordt niet meege-POST, dus de POST ziet dan waarde 0 -->
    ...
    <input type = checkbox class="delete" name = <?php echo "chbSkip_$Id" ; ?> value = 1 <?php echo ($check == 1 || $skip == 1) ? 'checked' : ''; if ($check == 1) {
?> disabled <?php
                                             } ?>  >
 </td>
