...
$taken = array('Worpregistratie', 'Doodgeboren', 'Groepsgeboorte', 'Verplaatsing', 'Spenen', 'Tussenweging', 'Afvoer', 'Aanvoer', 'Omnummeren', 'Medicaties', 'Halsnummers', 'Groepsafvoer', 'Voerregistratie', 'Dekken', 'Dracht', 'StallijstScan');
...
if($i == 7) { $inhoud = $item -> {$taken[$i]}; $velden = array('Datum', 'Ubn', 'RasId', 'HokId', 'Transponder', 'Levensnummer','Datumdier', 'Geslacht', 'ActId', 'Gewicht');
...
if($i == 12) { $inhoud = $item -> {$taken[$i]}; $velden = array('ActId', 'Datum', 'HokId', 'DoelId', 'ArtId', 'Toedat');
												include "impAgrident.php"; }

if($i == 13) { $inhoud = $item -> {$taken[$i]}; $velden = array('ActId', 'Datum', 'VdrId', 'MoederTransponder', 'Moeder');
												include "impAgrident.php"; }

if($i == 14) { $inhoud = $item -> {$taken[$i]}; $velden = array('ActId', 'Datum', 'MoederTransponder', 'Moeder', 'Drachtig', 'Grootte');
												/* $velden_dieren = array('MoederTransponder', 'Moeder', 'Drachtig', 'Grootte');
												include "impAgrident_dieren.php";*/ 
												include "impAgrident.php"; }
if($i == 15) { $inhoud = $item -> {$taken[$i]}; $velden = array('ActId', 'Datum', 'RasId', 'DoelId', 'HokId', 'Transponder', 'Levensnummer', 'vdrId', 'Geslacht');
												include "impAgrident.php"; }
echo $i.'<br>';
			 } 
