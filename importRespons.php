...
#echo '<br> kopieer Response '.$responsfile.' van '.$bright_map.' naar '.$dir."/";
copy($bright_map.$responsfile , $dir."/".$responsfile); // $responsfile is gedeclareerd in responscheck.php


if($code == 'VMD') { // $code is gedeclareerd in responscheck.php
...
if (!isset($dir) or empty($dir)) 									{ $fout = "De oorspronkelijke locatie wordt niet gevonden."; }
if (!isset($responsfile) or empty($responsfile)) 	{ if (isset($fout))	{ $fout .= "en het responsbestand wordt niet gevonden."; }													  else 				{ $fout  = 	  "Het responsbestand wordt niet gevonden."; } }
if (!isset($requestfile) or empty($requestfile)) 	{ if (isset($fout))	{ $fout .= " en het requestbestand wordt niet gevonden."; }													  else 				{ $fout  =	   "Het requestbestand wordt niet gevonden."; } }
// $requestfile is gedeclareerd in responscheck.php

if (!isset($fout)) { // Als controles zijn doorstaan

// Inlezen response bestand in tabel impRespons #echo '<br><br> response bestand wordt ingelezen. <br>';
$lok = $dir.'/'. $responsfile;

$inhoud = file($lok); //var_dump($inhoud);

$c = count($inhoud); //echo '<br>'.'<br>'.$c.'<br>'.'<br>';
...
$waarde = str_replace("'", "''", $regel[$ii]);
	...
	if($ii == 0) { $insert_qry = " INSERT INTO impRespons SET ".$velden[$ii]." = '".$waarde."'"; }
	else if($regel[$ii] == "" || $regel[$ii] == "0") {   $insert_qry .= ", ".$velden[$ii]." = NULL"; }
	else {   $insert_qry .= ", ".$velden[$ii]." = '".$waarde."'"; }
}

$insert_qry .= ';';

// echo $insert_qry.'<br><br>';
...
{ 
 $DelFile = $dir."/".$responsfile;  #echo '<br> verwijder Response '.$DelFile;
...
if ($zoek_response_BrightMap == 1) // Als response bestand bestaat in Bright map
{ 
// filedatum van response bestand ophalen
$pad = $bright_map.$responsfile; //echo '$pad = '.$pad.'<br>';
...
$datum = $dag.$maand[$mnd].$jaar.' '.$tijd.'u.'; //echo "$responsfile is het laatst gewijzigd op: " . $datum;
$date = $jaar.'-'.$mnd.'-'.$dag.' '.$tijdstip;
$make_date = date_create($date); $response_filedate = date_format($make_date, 'Y-m-d H:i:s');
// Einde filedatum van response bestand ophalen

 // Archiveren van response bestand bijv 1507131_bas_4_respons.txt  #echo '<br> kopieer Response '.$responsfile.' van /BRIGHT/ naar /user_'.$lidId.'/';
...
if ($zoek_request_BrightMap == 1) // Als request bestand bestaat in Bright map
{ 

// filedatum van request bestand ophalen
$pad = $bright_map.$requestfile; //echo '$pad = '.$pad.'<br>';
...
$datum_requestfile = $dag.$maand[$mnd].$jaar.' '.$tijd.'u.'; //echo "$responsfile is het laatst gewijzigd op: " . $datum;
$date = $jaar.'-'.$mnd.'-'.$dag.' '.$tijdstip;
$make_date = date_create($date); $request_filedate = date_format($make_date, 'Y-m-d H:i:s');
// Einde filedatum van request bestand ophalen

 // Archiveren van request bestand bijv 1507131_bas_4_request.txt 
 #echo '<br> kopieer Request '.$requestfile.' van /BRIGHT/ naar /user_'.$lidId.'/ <br>';
...
while ($resp = mysqli_fetch_assoc($zoek_status_response)) {	$def_resp = $resp['def']; $meldnr = $resp['meldnr']; }
