...
$queryInlog = mysqli_query($db,"SELECT relnr, ubn, urvo, prvo FROM tblLeden WHERE lidId = ".mysqli_real_escape_string($db,$lidId)." ") or die (mysqli_error($db));
...
function aantal_melden($datb,$lidid,$fldCode) {
	...
	where st.lidId = ".mysqli_real_escape_string($datb,$lidid)." and h.skip = 0 and isnull(r.dmmeld) and code = '$fldCode'
