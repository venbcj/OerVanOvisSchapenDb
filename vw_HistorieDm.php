...
$vw_HistorieDm =
("
SELECT st.stalId, s.schaapId, h.datum, a.actie, h.actId
FROM tblSchaap s
...
SELECT st.stalId, mdr.schaapId, min(h.datum) datum, 'Eerste worp' actie, NULL actId
FROM tblSchaap mdr
...
GROUP BY st.stalId, mdr.schaapId

Union

SELECT st.stalId, mdr.schaapId, max(h.datum) datum, 'Laatste worp' actie, NULL actId
FROM tblSchaap mdr
 join tblVolwas v on (mdr.schaapId = v.mdrId)
 join tblSchaap lam on (v.volwId = lam.volwId)
 join tblStal st on (st.schaapId = lam.schaapId)
 ...
 join tblHistorie h on (st.stalId = h.stalId and h.actId = 1 and h.skip = 0)
WHERE u.lidId = '".mysqli_real_escape_string($db,$lidId)."'
GROUP BY st.stalId, mdr.schaapId, h.actId
HAVING (max(h.datum) > min(h.datum))

Union

SELECT st.stalId, s.schaapId, p.dmafsluit datum, 'Gevoerd' actie, NULL actId
FROM tblVoeding vd
...
GROUP BY st.stalId, s.schaapId, p.dmafsluit


Union

SELECT st.stalId, s.schaapId, max(r.dmmeld) dmmeld, 'Geboorte gemeld' actie, h.actId
FROM tblMelding m 
...
GROUP BY st.stalId, s.schaapId

Union

SELECT st.stalId, s.schaapId, max(r.dmmeld) dmmeld, 'Aanvoer gemeld' actie, h.actId
FROM tblMelding m 
 join tblRequest r on (m.reqId = r.reqId)
 join tblHistorie h on (m.hisId = h.hisId)
 join tblStal st on (st.stalId = h.stalId)
...
SELECT st.stalId, s.schaapId, max(r.dmmeld) dmmeld, 'Afvoer gemeld' actie, h.actId
FROM tblMelding m 
 join tblRequest r on (m.reqId = r.reqId)
 join tblHistorie h on (m.hisId = h.hisId)
 join tblStal st on (st.stalId = h.stalId)
...
SELECT st.stalId, s.schaapId, max(r.dmmeld) dmmeld, 'Uitval gemeld' actie, h.actId
FROM tblMelding m 
 join tblRequest r on (m.reqId = r.reqId)
 join tblHistorie h on (m.hisId = h.hisId)
 join tblStal st on (st.stalId = h.stalId)
