...
// Einde Id ophalen
    foreach ($id as $key => $value) {
        if ($key == 'chbkies' /*&& $value == 1*/) {
         /*$box = $value ; */ $fldKies = $value;
        }
        if ($key == 'chbDel' /*&& $value == 0*/) {
            $fldDel = $value;
        }
        if ($key == 'chbKg' /*&& $value == 0*/) {
            $fldWeeg = $value;
        }
        ...
        if ($key == 'txtlevafl' && !empty($value)) {
     /*echo $key.'='.$value.' ';*/ $fldLevnr = $value;
        }
        if ($key == 'txtKg' && !empty($value)) {
     /*echo $key.'='.$value.' ';*/ $fldKg = str_replace(',', '.', $value);
        }
        if ($key == 'kzlBest' && !empty($value)) {
     /*echo $key.'='.$value.' ';*/ $fldBest = $value;
        }
        if ($key == 'kzlReden' && !empty($value)) {
     /*echo $key.'='.$value.' ';*/ $fldReduId = $value;
        }
    }
...
") or die(__FILE__ . ' (' . __LINE__ . ') ' . mysqli_error($db));
                while ($awId = mysqli_fetch_assoc($zoek_aanwas)) {
                    $aanwas = $awId['hisId'];
                }
                if (isset($aanwas)) {
                     $actId = 13;
...
WHERE stalId = '" . mysqli_real_escape_string($db, $stalId) . "' ";
            /*echo $update_tblStal.'<br>';*/    mysqli_query($db, $update_tblStal) or die(__FILE__ . ' (' . __LINE__ . ') ' . mysqli_error($db));
