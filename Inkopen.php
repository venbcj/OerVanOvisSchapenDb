...
if (empty($_POST['txtCharge_']))	{	$insCharge = 'NULL';	}
else		{	$insCharge ="$_POST[txtCharge_]";	}

if (empty($_POST['txtArtikel_']))	{	$insVoer = "NULL";	}
else		{	$insVoer =" $_POST[txtArtikel_] ";	}
...
") or die (mysqli_error($db));
	while ($ln= mysqli_fetch_assoc($ophalen_waardes)) {
		...
		if($insCharge == 'NULL') { 
			$insert_tblInkoop = "INSERT INTO tblInkoop SET 
				dmink = '".mysqli_real_escape_string($db,$insInkdm)."', 
				artId = ".mysqli_real_escape_string($db,$insVoer).", 
				charge = ".mysqli_real_escape_string($db,$insCharge).", 
				inkat = ".mysqli_real_escape_string($db,$insInkat).", 
				enhuId = ".mysqli_real_escape_string($db,$enhuId).", 
				prijs = ".mysqli_real_escape_string($db,$insPrijs).", 
				btw = ".mysqli_real_escape_string($db,$insBtw).", 
				relId = ".mysqli_real_escape_string($db,$insRc)." "; 
			}
		else {
			$insert_tblInkoop = "INSERT INTO tblInkoop SET 
				dmink = '".mysqli_real_escape_string($db,$insInkdm)."',
...
$PrijsInclBtw = $insPrijs + $btwBedrag;
		$insert_tblOpgaaf = "INSERT INTO tblOpgaaf SET rubuId = '$rubuId', datum = '$insInkdm', bedrag = '$PrijsInclBtw', toel = '".$relatie."', liq = 1 ";
...
<form action="Inkopen.php" method="post" >
 ...
 <tr style =  "font-size:12px;" valign =  "bottom"> 
		 ...
		 <th></th> 
		 <th width = 50>Stuksprijs (excl.)<hr></th>
		 <th></th> 
		 <th>Prijs (excl.)<hr></th> 
		 <th></th>
		 <th>Btw<hr></th>
		 ...
		 <th>Verwijder<hr></th> 
		 <th width = 100 ><input type = "submit" name = "knpSave_" value = "Opslaan" style = "font-size:9px" ><hr></th> 
...
WHERE eu.lidId = ".mysqli_real_escape_string($db,$lidId)." and i.inkId = ".mysqli_real_escape_string($db,$Id)."
...
<tr >
 ...
 <td width = "1"></td>
 <td align = right > &euro;&nbsp <input type = text name = <?php echo "txtPrijs_$Id"; ?> size = 4 style = "font-size:11px; text-align:right;" value = <?php	echo $prijs;?> ></td>
 <td width = "1"></td>
...
<table border= 0>
<tr><td colspan = 15 style = "color : red;"></td></tr>
...
<tr><td><input id = "datepicker1" type="text" name = "txtInkdm_" id = "datum" size = 8 value = <?php if(isset($inkdatum)) { echo $inkdatum; } ?> ></td>
<td></td>
<td>
