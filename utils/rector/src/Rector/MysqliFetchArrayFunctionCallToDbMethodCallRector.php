 ...
 */
final class MysqliFetchArrayFunctionCallToDbMethodCallRector extends AbstractRector
{
...
CODE_SAMPLE
    ...
    {
        if ($this->getName($node->name) != 'mysqli_fetch_array') {
            return null;
        }
        $newnode = new MethodCall(
            $node->args[0]->value,
            new Identifier('fetch_array')
        );
