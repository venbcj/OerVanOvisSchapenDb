...
function date_add_months($day,$var) { // zie Newuser.php
...
function zoek_schaapId_in_database($LEVNR) { // zie post_readerStalscan.php
global $db;
...
function zoek_transponder_in_database($LEVNR) { // zie post_readerStalscan.php
global $db;
...
WHERE levensnummer = '".mysqli_real_escape_string($db,$LEVNR)."' and lidId = '" . mysqli_real_escape_string($db,$LIDID) . "' and isnull(rel_best)
") or die (mysqli_error($db));
...
while ( $zhst = mysqli_fetch_assoc($zoek_hisId_stal)) { return $zhst['hisId']; }
}
...
while ( $zmhst = mysqli_fetch_assoc($zoek_max_hisId_stal)) { return $zmhst['hisId']; }
}
...
while ( $zhsa = mysqli_fetch_assoc($zoek_hisId_stal_af)) { return $zhsa['hisId']; }
}
...
while ( $zhs = mysqli_fetch_assoc($zoek_hisId_schaap)) { return $zhs['hisId']; }
}
...
while ( $zd = mysqli_fetch_assoc($zoek_datum)) { return $zd['datum1']; }
}
