...
haf.actie, lower(haf.actie) status, s.momId, s.redId,
b.bezId, ho.hoknr hoknr_lst, h_in.datum dmHokIn, date_format(h_in.datum,'%d-%m-%Y') hokInDm, p.periId, p.dmafsluit, 
st.rel_best, haf.hisId afvhisId, date_format(haf.datum,'%d-%m-%Y') afvoerdm, haf.kg afvoerkg
...
FROM tblSchaap s
 ...
 join (
	...
	GROUP BY st.stalId
 ) h_max on (stm.stalId = h_max.stalId)
...
/* BESTEMMING */		/* kzl afnemer tonen bij afgevoerde schapen m.u.v. overledenen en bij moeder- en vaderdieren */
  if( (isset($Actie) && $actie != 'overleden' && $actie != 'vermist') || (($fase == 'moederdier' || $fase == 'vaderdier') && !isset($Actie)) ) { ?>
<tr>
...
// OPTIE 5 = Afleveren, verkopen of afvoeren
if( (($actie == 'overleden' || $actie == 'uitgeschaard' || $actie == 'vermist') && !isset($dmaanw) && isset($spndm)) || (!isset($afvhis) && $modtech == 0)		)				  		  { $new_afvoer = 'Afleveren'; $value = 12;} 
else if( (($actie == 'overleden' || $actie == 'uitgeschaard' || $actie == 'vermist') && isset($dmaanw)) || (isset($dmaanw) && !isset($afvhis)) ) { $new_afvoer = 'Verkopen'; $value = 13; }
...
// OPTIE 7 = Vermissing
if($actie != 'vermist' || !isset($afvhis)) 		{ $optie7 = 'Vermist'; }
