...
09-08-2019 www. weggehaald bij url 
04-09-2023 Datum Laatste stalscan toegevoegd 
01-01-2024 and h.skip = 0 aangevuld aan tblHistorie */
...
if($p == 1) {
	...
	$this->Cell(15,5,'',0,0,'C',false);
	$this->Cell(25,5,'',0,0,'C',false);
	$this->Cell(15,5,'',0,0,'C',false);
	$this->Cell(15,5,'',0,0,'C',false);
	$this->Cell(15,5,'',0,0,'C',false);
	$this->Cell(15,5,'Laatste',0,1,'C',false);
	...
	$this->Cell(15,5,'Generatie',0,0,'C',false);
	$this->Cell(15,5,'stalscan',0,1,'C',false);
...
SELECT s.levensnummer, right(s.levensnummer, $Karwerk) werknum, date_format(hg.datum,'%d-%m-%Y') gebdm, s.geslacht, prnt.datum aanw, date_format(contr.datum,'%d-%m-%Y') maxScandm
FROM tblSchaap s
 join tblStal st on (st.schaapId = s.schaapId)
 ...
 left join (
	...
	FROM tblStal st
	 join tblHistorie h on (st.stalId = h.stalId)
	WHERE h.actId = 22 and h.skip = 0
	GROUP BY st.schaapId
 ) contr on (contr.schaapId = s.schaapId) 
WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and isnull(st.rel_best)
...
while($row = mysqli_fetch_array($result))
	   ...
	   $pdf->SetDrawColor(200,200,200); // Grijs
		...
		$pdf->Cell(15,5,$fase,'TB',0,'C',false);
		$pdf->Cell(15,5,$maxScandm,'TB',1,'C',false);
