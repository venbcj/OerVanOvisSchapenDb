...
- Generatie verplicht, omdat een lam in een verblijf geplaatst wordt.
...
$velden = "rd.actId, rd.Id readId, rd.datum, rd.levensnummer levnr_rd, coalesce(r.ras,'onbekend') ras, s.lidId, s.geslacht,  
 ouder.ouder,
 af.actie,
 hk.hoknr,
s.levensnummer levnr_stal, hg.gebdm, dup.dubbelen ";

$tabel = "
impAgrident rd
 left join (
	SELECT max(h.hisId) hisId, st.stalId, s.schaapId, s.levensnummer, s.geslacht, s.rasId, st.lidId
	FROM tblSchaap s
...
<table> <!-- Deze tabel bevat een linker tabel met in te lezen dieren uit de reader en een rechter tabel met dieren p de stallijst die niet zijn gescand -->
<tr>
<td valign="top">
...
<tr style = "font-size : 12px;">
 ...
 <th valign = bottom style = "font-size : 11px;">R<br>V<br>&nbspO*<hr></th>
<?php } ?>
...
$datum2 = date_format(date_create($date2), 'd-m-Y');
	 ...
	 If	( 		
							...
							empty($scandatum)					|| # of datum is leeg
							isset($afvoer)						|| # of het dier is reeds geregistreerd als afgevoerd


	( $levnr_stal == 0 && (// Als het levensnummer niet op de stallijst staat

	 									empty($kzlActie)										 || // Als de actie niet bekend is
	 									($kzlActie == 1 && empty($txtGebdm)) || // Als geboorte wordt vastgelegd zonder geboortedatum
(isset($txtDmgeb) && isset($date2) && $date2 < $txtDmgeb) || // Als geboorte groter is dan eerst volgende historie
	 						(!isset($schaapId_db) && empty($kzlFase))  || // Als generatie niet is gevuld
	 									($kzlFase == 'lam' && empty($kzlHok))|| // Bij een lam is het verblijf verplicht
($kzlActie == 1 && ($kzlFase == 'moeder' || $kzlFase == 'vader') )	|| // Generatie en actie zijn tegenstrijdig
						($kzlSekse == 'ooi' && $kzlFase == 'vader')	 || // Geslacht en generatie zijn tegenstrijdig
						($kzlSekse == 'ram' && $kzlFase == 'moeder') //|| // Geslacht en generatie zijn tegenstrijdig


) )// Einde Als het levensnummer niet op de stallijst staat
...
if (isset($levnr_dupl) ) 					{ $color = 'blue'; $bericht =  "Dubbel in de reader."; }
else if (isset($afvoer)) 					{ $color = 'red'; $bericht = "Dit schaap is reeds geregistreerd als ".strtolower($afvoer)."."; }  
...
else if (isset($txtDmgeb) && isset($date2) && $date2 < $txtDmgeb)	{ $color = 'red'; $bericht = "De geboortedatum mag niet na ".$datum2." liggen."; } 
...
<!-- <input type = "hidden" name = <p??hp echo " \"txtlevgeb_$Id\" value = \"$levnr_rd\" ;"?> size = 9 style = "font-size : 9px;"> -->
 ...
 <td style = "text-align:center;" width= 80>
 <?php if($levnr_stal > 0) { echo $geslacht_stal; } else if(isset($geslacht_db)) { echo $geslacht_db; } else if (!isset($bericht)) { ?>
<!-- KZLGESLACHT --> 
...
<?php if($levnr_stal > 0) { echo $fase; } else if(isset($fase_db)) { echo $fase_db; } else if (!isset($bericht)) { ?> 	
...
<?php if(isset($bericht)) { $colspan = 4; } else { $colspan = 1; } ?>

 <td colspan="<?php echo $colspan; ?>" style = "text-align:center; color : <?php echo $color; ?> ; font-size : 13px;" > <?php 
...
<?php }
} /* Einde else na if(isset($bericht))*/ ?>
</tr>
<!--	**************************************
...
<?php 
$zoek_niet_gescand = mysqli_query($db,"
...
FROM tblSchaap s
 ...
 left join (
 	SELECT levensnummer
	FROM impAgrident
	WHERE lidId = '" . mysqli_real_escape_string($db,$lidId) . "' and actId = 22 and datum = '" . mysqli_real_escape_string($db,$scandate) . "'
...
while ($zng = mysqli_fetch_assoc($zoek_niet_gescand)) 
{ $aant = $zng['nietgescand']; }

if($aant > 0) { $alles = 'Nee'; $tekst = 'Schapen op de stallijst die niet zijn gescand ' .'<br>'. ' op ' . $scandatum . '.'; }
else { $alles = 'Ja'; $tekst = 'De hele stallijst is gescand.'; }

?>

<table border = 0 >  <!-- Rechter tabel -->
<tr height = 50 > 
 <td colspan="5" width="350" align="center"> <?php echo $tekst; ?> </td>
</tr>
...
</table> <!-- Einde : Deze tabel bevat een linker tabel met in te lezen dieren uit de reader en een rechter tabel met dieren p de stallijst die niet zijn gescand -->
</form> 
