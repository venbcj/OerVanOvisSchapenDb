...
SELECT concat(transponder,levensnummer) tran, coalesce(s.geslacht,'Onbekend') geslacht, coalesce(r.ras,'Onbekend') ras, coalesce(lw.worp,'n.v.t.') lastworp, coalesce(lw.werpdm,'n.v.t.') lastwerpdm,
	...
	while($zi = mysqli_fetch_array($zoek_info))
		{
			...
			$lastwerpdm = $zi['lastwerpdm']; if($geslacht == 'ram') { $lastwerpdm = 'n.v.t.'; }
			...
			$opties[] = array('Transponder' => $zi['tran'], 'Geslacht' => $geslacht, 'Ras' => $zi['ras'], 'Laatsteworp' => $lastworp, 'Laatstewerpdm' => $lastwerpdm, 'Dekaantal' => $aantDek, 'Worpaantal' => $aantWorp, 'Gemiddeldeworp' => $gemWorp, 'Lamaantal' => $aantLam, 'PercLevend' => $PercLevend, 'Maxworp' => $maxWorp, 'Aantmaxworp' => $aantMaxWorp);
