...
$velden = "rd.actId, rd.Id readId, rd.datum, rd.levensnummer levnr_rd, coalesce(r.ras,'onbekend') ras, s.geslacht,  
 ouder.ouder,
s.levensnummer levnr_db, r.rasId ras_db, hg.gebdm, dup.dubbelen ";

$tabel = "
impAgrident rd
 left join (
	SELECT max(h.hisId) hisId, s.schaapId, s.levensnummer, s.geslacht, s.rasId
	FROM tblSchaap s
	 join tblStal st on (st.schaapId = s.schaapId)
	 join tblHistorie h on (st.stalId = h.stalId)
	WHERE st.lidId = '" . mysqli_real_escape_string($db,$lidId) . "' and h.skip = 0
	GROUP BY s.schaapId, s.levensnummer, s.geslacht, s.rasId
...
<table border = 0>
<tr> <form action="InsStallijstscan_controle.php" method = "post">
...
$datum = date('d-m-Y', strtotime($date));
	...
	$ouder = $array['ouder']; if(isset($ouder) && $sekse == 'ooi') { $fase = 'moeder'; } else if(isset($ouder) && $sekse == 'ram') { $fase = 'vader'; } else { $fase = 'lam'; }
 	$gebdm = $array['gebdm'];
	$actId = $array['actId'];
...
$datum = $_POST["txtScandm_$Id"]; $kzlRas = $_POST["kzlras_$Id"]; $sekse = $_POST["kzlsekse_$Id"]; $fase = $_POST["kzlFase_$Id"];
if($modtech == 1) { $kzlHok = $_POST["kzlhok_$Id"]; }
	 }
	 If	 
	 ( /*Scan moeders*/   ( ($fase == 'moeder' || $fase == 'vader') && #generatie moet moeder of vader zijn
	...
	||	
	/*Scan lammeren*/  (  $fase == 'lam' && #generatie moet lam zijn
						(	strlen($levnr_rd)<> 12		|| # of levensnummer is geen 12 karakters lang of dus leeg
							numeriek($levnr_rd) == 1	|| # of levensnummer bevat een letter  
	...
	||
	/*Scan niet moeders of lammeren*/  (   empty($fase) #generatie kan nooit vader of leeg zijn
					)
...
<tr style = "font-size:14px;">
 ...
 <td>
<?php if (isset($_POST['knpVervers_'])) { $datum = $_POST["txtScandm_$Id"]; } ?>
	<input type = "text" size = 9 style = "font-size : 11px;" name = <?php echo "txtScandm_$Id"; ?> value = <?php echo $datum; ?> >
 </td>
...
<!-- <input type = "hidden" name = <p??hp echo " \"txtlevgeb_$Id\" value = \"$levnr_rd\" ;"?> size = 9 style = "font-size : 9px;"> -->
  ...
  <td style = "text-align:center;" width= 80>
 	<?php if($levnr_db > 0) { echo $gebdm; } else { ?>

 	<?php } ?>
 </td>
 <td style = "text-align:center;" width= 80>
 	<?php if($levnr_db > 0) { echo $ras_db; } else { ?>
<!-- KZLRAS -->
...
for ($i = 0; $i < $count; $i++){
 ...
 ?> </select>
<?php if( !empty($rasId_rd) && empty($ras_db) && !isset($_POST['knpVervers_']) ) {echo $rasId_rd; ?> <b style = "color : red;"> ! </b> <?php } ?>
	 <!-- EINDE KZLRAS -->
<?php } ?>
 </td>
 <td style = "text-align:center;" width= 80>
 <?php if($levnr_db > 0) { echo $sekse; } else { ?>
<!-- KZLGESLACHT --> 
...
<?php if($levnr_db > 0) { echo $fase; } else { ?> 	
<!-- KZLGENERATIE --> 
<?php //echo "$fase"; ?>
