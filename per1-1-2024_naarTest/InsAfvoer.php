...
$versie = '15-6-2018';  /* $dmmax houdt geen rekening meer met melddatums bij RVO */
...
impAgrident rd
 ...
 left join (
	SELECT schaapId, max(datum) datummax
	FROM (
		...
		FROM tblSchaap s
			...
			join tblHistorie h on (h.stalId = st.stalId)
			join tblActie a on (a.actId = h.actId)
		WHERE a.actId = 5 and h.skip = 0 and st.lidId = '".mysqli_real_escape_string($db,$lidId)."'
		...
		FROM tblSchaap s
			...
			join tblHistorie h on (h.stalId = st.stalId)
			join tblActie a on (a.actId = h.actId)
		WHERE a.actId = 8 and h.skip = 0 and st.lidId = '".mysqli_real_escape_string($db,$lidId)."'
		...
		FROM tblSchaap s
			...
			join tblHistorie h on (h.stalId = st.stalId)
			join tblActie a on (a.actId = h.actId)
		WHERE a.actId = 9 and h.skip = 0 and st.lidId = '".mysqli_real_escape_string($db,$lidId)."'
		...
		FROM tblSchaap s
			...
			join tblHistorie h on (h.stalId = st.stalId)
			join tblActie a on (a.actId = h.actId)
		WHERE a.actId = 12 and h.skip = 0 and st.lidId = '".mysqli_real_escape_string($db,$lidId)."'
		...
		FROM tblSchaap s
			...
			join tblHistorie h on (h.stalId = st.stalId)
			join tblActie a on (a.actId = h.actId)
		WHERE a.actId = 13 and h.skip = 0 and st.lidId = '".mysqli_real_escape_string($db,$lidId)."'
		...
		FROM tblSchaap s
			...
			join tblHistorie h on (h.stalId = st.stalId)
			join tblActie a on (a.actId = h.actId)
		WHERE a.actId = 14 and h.skip = 0 and st.lidId = '".mysqli_real_escape_string($db,$lidId)."'
...
$velden = "rd.readId, rd.datum, right(rd.levnr_afv,".mysqli_real_escape_string($db,$Karwerk).") werknr, rd.levnr_afv levnr, rd.ubn_afv, r.ubn ctrubn, NULL redId_rd, NULL reduId_db, round((rd.afvoerkg/10),2) kg, s.schaapId, s.geslacht, ouder.datum dmaanw, lower(haf.actie) actie, haf.af, hs.datum dmspeen, ak.datum dmaankoop, date_format(max.datummax,'%d-%m-%Y') maxdatum, max.datummax, b.bezId ";
...
impReader rd
 left join (
	SELECT s.schaapId, s.levensnummer, s.geslacht
	 ...
	 WHERE st.lidId = '".mysqli_real_escape_string($db,$lidId)."'
	 GROUP BY s.schaapId, s.levensnummer, s.geslacht
 ) s on (s.levensnummer = rd.levnr_afv)
 left join (
	SELECT st.schaapId, h.hisId, a.actie, a.af
	FROM tblStal st
	 join tblHistorie h on (st.stalId = h.stalId)
 ...
 left join (
	...
	WHERE st.lidId = '".mysqli_real_escape_string($db,$lidId)."' and h.actId = 2 and h.skip = 0
	GROUP BY levensnummer
 ) ak on (ak.levensnummer = rd.levnr_afv)
 left join (
	SELECT schaapId, max(datum) datummax
	FROM (
SELECT s.schaapId, h.datum, a.actie, h.actId, h.skip
...
GROUP BY s.schaapId, p.dmafsluit
 ...
 left join (
	...
	FROM tblPartij p
	 join tblRelatie r on (p.partId = r.partId)
	WHERE p.actief = 1 and r.relatie = 'deb' and r.actief = 1
 ) r on(r.ubn = rd.ubn_afv and r.lidId = rd.lidId)
 left join (
	SELECT max(b.bezId) bezId, s.levensnummer
	FROM tblBezet b
	 join tblHistorie h on (b.hisId = h.hisId)
	...
	WHERE st.lidId = '".mysqli_real_escape_string($db,$lidId)."' and h.skip = 0
	GROUP BY s.levensnummer
 ) b on (rd.levnr_afv = b.levensnummer)
";

$WHERE = "WHERE rd.lidId = '".mysqli_real_escape_string($db,$lidId)."' and rd.teller_afv is not null and isnull(rd.verwerkt) ";

include "paginas.php";

$data = $page_nums-> fetch_data($velden, "ORDER BY right(rd.levnr_afv,".mysqli_real_escape_string($db,$Karwerk).") "); 
...
// Controleren of ingelezen waardes worden gevonden .
 $kzlRelatie = $ubn_db; 
if (isset($_POST['knpVervers_'])) { $datum = $_POST["txtAfvoerdag_$Id"]; if(isset($_POST["txtKg_$Id"])) { $kg = $_POST["txtKg_$Id"]; } $kzlRelatie = $_POST["kzlBest_$Id"]; 
	$makeday = date_create($_POST["txtAfvoerdag_$Id"]); $date =  date_format($makeday, 'Y-m-d'); }

	 If	 
	 ( !isset($schaapId) || $status == 'overleden' || $status == 'afgeleverd' || /*levensnummer moet aanwezig zijn */
...
<?php if( $red_rd <> NULL && empty($reduId_db) && empty($_POST["kzlReden_$Id"]) ) { ?> <b style = "color : red;"> ! </b>  <?php } ?>
 ...
 <td width = 80 ><center><?php 
if (isset($status)) { echo $fase ;} ?></center>
 </td> <?php
// Wachtdagen bepalen
if(isset($schaapId)) {
...
<!-- Foutmeldingen -->
 <td colspan = 2 width = 300 style = "color : red"> <?php
	...
	else if($status == 'overleden' || $status == 'afgeleverd')   { echo "Dit schaap is reeds $status."; } 
	else if(isset($fase) && $date < $dmmax)   { echo "Datum ligt voor $maxdm."; } 
