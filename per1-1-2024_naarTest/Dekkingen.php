...
else if(isset($vroegst_volgende_dekdatum) && $vroegst_volgende_dekdatum > $txtDay) { $fout = "Deze ooi is heeft binnen het laatste half jaar nog geworpen. "; }
...
FROM (
 ...
 join (
	...
	FROM tblVolwas
	WHERE vdrId = '". mysqli_real_escape_string($db,$kzlVdr) ."'
	GROUP BY mdrId 
...
De gekozen dekdatum mag niet voor de laatste dekdatum liggen. Bij hoerveel dieren dit het geval is in het verblijfdoet er niet toe. Vandaar geen HAVING (count(mdrs.mdrId)) >= 5 in de query hierboven. 

Zijn de ooien binnen het verblijf als laatst gedekt door het gekozen vaderdier dan moeten er minimaal 5 moeders een koppel zijn met het gekozen vaderdier om te beoordelen dat dit verblijf de laatste keer reeds is gedekt door het gekozen vaderdier. Vandaar HAVING (count(mdrs.mdrId)) >= 5 in de query hieronder.
...
unset($max_worp);
unset($dmwerp_plus_183dgn);
// Controle dekking binnen 183 dagen na laatste worp per moeder
...
SELECT date_add(max(h.datum),interval 183 day) werpdate
FROM tblSchaap s
 join tblStal st on (s.schaapId = st.schaapId)
 join tblHistorie h on (h.stalId = st.stalId)
WHERE h.actId = 1 and h.skip = 0 and s.volwId = '" . mysqli_real_escape_string($db,$max_worp) . "'
") or die (mysqli_error($db));
	while ($zlw = mysqli_fetch_assoc($zoek_laatste_werpdatum)) { $dmwerp_plus_183dgn = $zlw['werpdate']; }

if($dmwerp_plus_183dgn > $txtDay) { 
...
") or die (mysqli_error($db));
	...
	if($foutaantal > 5) { 
	$fout = $foutaantal." ooien hebben binnen het laatste half jaar nog geworpen. ".'\n'."Deze ".$foutaantal." dieren zijn daarom niet gedekt.";
	}
	else {
	$fout .= 'De ooi ' .$werknr_ooi. ' heeft binnen het laatste half jaar nog geworpen en is daarom niet gedekt. \n'; 
	}
...
// Einde Controle dekking binnen 183 dagen na laatste worp per moeder

if(!isset($max_worp) || (isset($dmwerp_plus_183dgn) && $dmwerp_plus_183dgn <= $txtDay)) {
