...
FROM (
	...
	FROM tblRubriek r
	 ...
	 left join (
		...
		WHERE year(l.datum) = '". mysqli_real_escape_string($db,$kzlJaar) ."'
	 ...
	 left join (
		...
		WHERE ru.lidId = '". mysqli_real_escape_string($db,$lidId) ."' and year(o.datum) = '". mysqli_real_escape_string($db,$kzlJaar) ."'
		GROUP BY o.rubuId, date_format(o.datum,'%Y')
	 ) o on (o.rubuId = ru.rubuId)
	WHERE ru.lidId = '". mysqli_real_escape_string($db,$lidId) ."' and sb.tbl = 'ru' and year(sb.datum) = '". mysqli_real_escape_string($db,$kzlJaar) ."' and r.actief = 1 and ru.sal = 1
	...
	SELECT r.credeb, sum(coalesce( '". mysqli_real_escape_string($db,$p_ooital) ."' *sb.waarde,0)) bedrag_slb, sum(l.bedrag) bedrag_liq, sum(coalesce(o.bedrag,0)) bedrag_real
	FROM tblRubriek r
	 join tblRubriekuser ru on (r.rubId = ru.rubId)
	 join tblSalber sb on (sb.tblId = ru.rubuId)
	 left join (
	...
	SELECT r.credeb, sum(coalesce( '". mysqli_real_escape_string($db,$p_afv) ."' * sb.waarde,0)) bedrag_slb, sum(l.bedrag) bedrag_liq, sum(coalesce(o.bedrag,0)) bedrag_real
	FROM tblRubriek r
	 join tblRubriekuser ru on (r.rubId = ru.rubId)
	 join tblSalber sb on (sb.tblId = ru.rubuId)
	 left join (
	...
	SELECT r.credeb, sum(coalesce( '". mysqli_real_escape_string($db,$verv_ooi*$p_ooital/100) ."' *sb.waarde,0)) bedrag_slb, sum(l.bedrag) bedrag_liq, sum(coalesce(o.bedrag,0)) bedrag_real
	FROM tblRubriek r
	 join tblRubriekuser ru on (r.rubId = ru.rubId)
	 join tblSalber sb on (sb.tblId = ru.rubuId)
	 left join (
	...
	FROM tblRubriek r
	 ...
	 left join (
		...
		WHERE ru.lidId = '". mysqli_real_escape_string($db,$lidId) ."' and year(l.datum) = '". mysqli_real_escape_string($db,$kzlJaar) ."' and ru.sal = 1
	 ...
	 left join (
		...
		FROM tblOpgaaf o
		 join tblRubriekuser ru on (o.rubuId = ru.rubuId)
		WHERE ru.lidId = '". mysqli_real_escape_string($db,$lidId) ."' and year(o.datum) = '". mysqli_real_escape_string($db,$kzlJaar) ."' and ru.sal = 1
	...
	SELECT r.credeb, -sum(coalesce(sb.aantal,0)*coalesce(sb.waarde,0)) bedrag_slb, -sum(coalesce(l.bedrag,0)) bedrag_liq, -sum(coalesce(o.bedrag,0)) bedrag_real
	FROM tblRubriek r
	 join tblRubriekuser ru on (r.rubId = ru.rubId)
	 join tblSalber sb on (sb.tblId = ru.rubuId)
	 left join (
	...
	SELECT r.credeb, -sum( '". mysqli_real_escape_string($db,$p_ooital) ."' * coalesce(sb.waarde,0)) bedrag_slb, -sum(coalesce(l.bedrag,0)) bedrag_liq, -sum(coalesce(o.bedrag,0)) bedrag_real
	FROM tblRubriek r
	 join tblRubriekuser ru on (r.rubId = ru.rubId)
	 join tblSalber sb on (sb.tblId = ru.rubuId)
	 left join (
	...
	SELECT r.credeb, -sum(coalesce( '". mysqli_real_escape_string($db,$p_ooital) ."' * sb.aantal,0)*coalesce(sb.waarde,0)) bedrag_slb, -sum(coalesce(l.bedrag,0)) bedrag_liq, -sum(coalesce(o.bedrag,0)) bedrag_real
	FROM tblRubriek r
	 join tblRubriekuser ru on (r.rubId = ru.rubId)
	 join tblSalber sb on (sb.tblId = ru.rubuId)
	 left join (
	...
	SELECT r.credeb, -sum( '". mysqli_real_escape_string($db,$p_afv) ."' * coalesce(sb.waarde,0)) bedrag_slb, -sum(coalesce(l.bedrag,0)) bedrag_liq, -sum(coalesce(o.bedrag,0)) bedrag_real
	FROM tblRubriek r
	 join tblRubriekuser ru on (r.rubId = ru.rubId)
	 join tblSalber sb on (sb.tblId = ru.rubuId)
	 left join (
	...
	SELECT r.credeb, -sum(coalesce( '". mysqli_real_escape_string($db,$p_afv) ."' * sb.aantal,0)*coalesce(sb.waarde,0)) bedrag_slb, -sum(coalesce(l.bedrag,0)) bedrag_liq, -sum(coalesce(o.bedrag,0)) bedrag_real
	FROM tblRubriek r
	 join tblRubriekuser ru on (r.rubId = ru.rubId)
	 join tblSalber sb on (sb.tblId = ru.rubuId)
	 left join (
	...
	SELECT r.credeb, -sum( '". mysqli_real_escape_string($db,$verv_ooi*$p_ooital/100) ."' * coalesce(sb.waarde,0)) bedrag_slb, -sum(coalesce(l.bedrag,0)) bedrag_liq, -sum(coalesce(o.bedrag,0)) bedrag_real
	FROM tblRubriek r
	 join tblRubriekuser ru on (r.rubId = ru.rubId)
	 join tblSalber sb on (sb.tblId = ru.rubuId)
	 left join (
