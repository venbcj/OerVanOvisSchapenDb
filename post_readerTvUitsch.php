...
WHERE Id = '" . mysqli_real_escape_string($db, $recId) . "'
") or die(__FILE__ . ' (' . __LINE__ . ') ' . mysqli_error($db));
        while ($lv = mysqli_fetch_assoc($zoek_levnr)) {
            $dbLevnr = $lv['levnr_aanv'];
            $transp_rd = $lv['transponder'];
        }
...
");
            while ($zlksa = mysqli_fetch_assoc($zoek_laatste_keer_van_stallijst_af)) {
                     $max_his_af = $zlksa['hisId'];
            }
            unset($stalId_uitsch);
...
");
            while ($zub = mysqli_fetch_assoc($zoek_ubn_bestemming)) {
                     $ubn_best = $zub['ubn'];
                $partij = $zub['partij'];
            }
            unset($relId_herk);
...
");
            while ($zcu = mysqli_fetch_assoc($zoek_crediteur_van_ubn)) {
                     $fldHerk = $zcu['relId'];
            }
        ...
        /*echo $insert_tblHistorie_aank.'<br>';*/        mysqli_query($db, $insert_tblHistorie_aank) or die(__FILE__ . ' (' . __LINE__ . ') ' . mysqli_error($db));
            if (isset($fldHok)) {
