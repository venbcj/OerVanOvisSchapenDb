...
case 'POST':
    ...
    foreach ($data as $index => $item) {
        ...
        for ($i = 0; $i < count($taken); $i++) { // Er zijn 7 elementen nl. zie array $velden
            ...
            if ($i == 7) {
                $inhoud = $item -> {$taken[$i]};
                $velden = array('Datum', 'UbnId', 'Ubn', 'RasId', 'HokId', 'Transponder', 'Levensnummer','Datumdier', 'Geslacht', 'ActId', 'Gewicht');
            ...
            if ($i == 12) {
                $inhoud = $item -> {$taken[$i]};
                $velden = array('ActId', 'Datum', 'UbnId', 'Ubn', 'HokId', 'Transponder', 'Levensnummer');
                include "impAgrident.php";
            }
            if ($i == 13) {
                $inhoud = $item -> {$taken[$i]};
            ...
            if ($i == 16) {
                $inhoud = $item -> {$taken[$i]};
                $velden = array('ActId', 'Datum', 'UbnId', 'HokId', 'Transponder', 'Levensnummer');
                include "impAgrident.php";
            }
            if ($i == 17) {
                $inhoud = $item -> {$taken[$i]};
            ...
            if ($i == 18) {
                $inhoud = $item -> {$taken[$i]};
                $velden = array('ActId', 'Datum', 'UbnId', 'RasId', 'DoelId', 'HokId', 'Transponder', 'Levensnummer', 'vdrId', 'Geslacht');
                include "impAgrident.php";
            }
            echo $i . '<br>';
        }
