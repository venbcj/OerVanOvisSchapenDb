<?php
/* 7-6-2020 : In backupnaam mili seconden toegevoegd.
...
if(isset ($_POST['knpUpload']) && $reader == 'Biocontrol')
{
	...
	if ($end_dir_exists == 0) {$fout = "De bestemmingslocatie, ".$end_dir_reader.", wordt niet gevonden. Raadpleeg de beheerder. ";}
		else {
    if(is_uploaded_file($_FILES['bestand']['tmp_name']))
    {
        ...
        if(in_array($pathinfo['extension'], $allowed))
        {
			...
			else { $fout = 'De bestandsnaam is onjuist. Hernoem het eerst naar  : '. $input_file; }
		}
        else
        {
...
else if (isset($_POST['knpUpload'])) { $fout = 'Handmatig inlezen is met deze reader niet mogelijk.'; }
?>
