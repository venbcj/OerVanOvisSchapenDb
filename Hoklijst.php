...
if (Auth::is_logged_in()) {
    ...
    $doel_gateway = new DoelGateway();
    $dgroep = $doel_gateway->zoek_doel($pstId);
?>
<table border = 0 >
<tr >
<td > </td>
...
<?php
    ...
    while ($hk = $zoek_hok_ingebruik->fetch_assoc()) {
        $hokId = $hk['hokId'];
        $hok = $hk['hoknr'];
?>
<table border = 0 >
...
<?php
        if ($pstId == 1) {
            $zoek_nu_in_verblijf = $bezet_gateway->hoklijst_zoek_nu_in_verblijf_geb($hokId);
        }

        if ($pstId == 2) {
            $zoek_nu_in_verblijf = $bezet_gateway->hoklijst_zoek_nu_in_verblijf_spn($hokId);
        }
?>
...
<?php
        while ($n = $zoek_nu_in_verblijf->fetch_assoc()) {
            $nu = $n['nu'];
?>
