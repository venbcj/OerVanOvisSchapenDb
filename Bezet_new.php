...
<form action = "Bezet_new.php" method = "post">
...
$zoek_verblijven = mysqli_query($db,"
SELECT hokId, hoknr
FROM tblHok
WHERE lidId = '".mysqli_real_escape_string($db,$lidId)."'
") or die (mysqli_error($db));

		while($row = mysqli_fetch_assoc($zoek_verblijven))
		{  // Loop alle verblijven in gebruik
		/*$periId = $row['periId'];*/
		$hokId = $row['hokId'];
		$hoknr = $row['hoknr'];
		
$zoek_doelgroep_1_in_verblijf_voor_en_na_afsluitdatum = mysqli_query($db,"

SELECT b.hokId, count(distinct st.schaapId) aant_geb
FROM tblBezet b
 join tblHistorie h on (b.hisId = h.hisId)
 join tblStal st on (st.stalId = h.stalId)
 left join 
...
if(mysqli_num_rows($zoek_doelgroep_1_in_verblijf_voor_en_na_afsluitdatum) > 0 ) { $array_hokIds .= $hokId.','; }
else {

$zoek_doelgroep_1_uit_verblijf_na_afsluitdatum = mysqli_query($db,"
SELECT b.hokId, count(distinct st.schaapId) aant_geb
FROM tblBezet b
 join tblHistorie h on (h.hisId = b.hisId)
 join 
 (
	...
	FROM tblBezet b
	 ...
	 join tblActie a2 on (a2.actId = h2.actId)
	 join tblStal st on (h1.stalId = st.stalId)
	WHERE b.hokId = '".mysqli_real_escape_string($db,$hokId)."' and a2.uit = 1 and h1.skip = 0 and h2.skip = 0 and h1.actId != 2
	GROUP BY b.bezId
 ) uit on (uit.bezId = b.bezId)
 join tblHistorie ht on (ht.hisId = uit.hist)
 join tblStal st on (st.stalId = h.stalId)
 ...
 left join (
	...
	FROM tblPeriode p
	 join tblHok h on (h.hokId = p.hokId)
	WHERE h.hokId = '".mysqli_real_escape_string($db,$hokId)."' and p.doelId = 1 and dmafsluit is not null
	GROUP BY p.hokId
 ) endgeb on (endgeb.hokId = b.hokId)
WHERE b.hokId = '".mysqli_real_escape_string($db,$hokId)."' and ht.datum > coalesce(dmstop,'1973-09-11') 
 and ( isnull(spn.schaapId)  or (spn.datum  > coalesce(dmstop,'1973-09-11') and 
 		( h.datum < spn.datum || (h.datum = spn.datum && h.hisId < spn.hisId) ) )
 	 )
 and ( isnull(prnt.schaapId) or (prnt.datum > coalesce(dmstop,'1973-09-11') and h.datum < prnt.datum) )
...
if(mysqli_num_rows($zoek_doelgroep_1_uit_verblijf_na_afsluitdatum) > 0 ) { $array_hokIds .= $hokId.','; }
else {

$zoek_doelgroep_2_in_verblijf_voor_en_na_afsluitdatum = mysqli_query($db,"
SELECT b.hokId, count(distinct st.schaapId) aant_spn
FROM tblBezet b
 join tblHistorie h on (b.hisId = h.hisId)
 join tblStal st on (st.stalId = h.stalId)
 left join 
...
if(mysqli_num_rows($zoek_doelgroep_2_in_verblijf_voor_en_na_afsluitdatum) > 0 ) { $array_hokIds .= $hokId.','; }
else {

$zoek_doelgroep_2_uit_verblijf_na_afsluitdatum = mysqli_query($db,"
SELECT b.hokId, count(distinct st.schaapId) aant_spn
FROM tblBezet b
 join tblHistorie h on (h.hisId = b.hisId)
 join 
 ...
 left join (
	...
	FROM tblPeriode p
	 join tblHok h on (h.hokId = p.hokId)
	WHERE h.hokId = '".mysqli_real_escape_string($db,$hokId)."' and p.doelId = 2 and dmafsluit is not null
	GROUP BY p.hokId
 ) endspn on (endspn.hokId = b.hokId)
WHERE b.hokId = '".mysqli_real_escape_string($db,$hokId)."' and ht.datum > coalesce(dmstop,'1973-09-11') 
 and (h.datum > spn.datum || (h.datum = spn.datum && h.hisId >= spn.hisId) )
...
if(mysqli_num_rows($zoek_doelgroep_2_uit_verblijf_na_afsluitdatum) > 0 ) { $array_hokIds .= $hokId.','; }
else {

$zoek_doelgroep_3_nu_in_verblijf = mysqli_query($db,"
SELECT b.hokId, count(distinct st.schaapId) aant_prnt
FROM tblBezet b
 join tblHistorie h on (b.hisId = h.hisId)
 join tblStal st on (st.stalId = h.stalId)
 left join 
...
if(mysqli_num_rows($zoek_doelgroep_3_nu_in_verblijf) > 0 ) { $array_hokIds .= $hokId.','; }
else {
...
FROM tblBezet b
 ...
 left join (
	...
	FROM tblPeriode p
	 join tblHok h on (h.hokId = p.hokId)
	WHERE h.hokId = '".mysqli_real_escape_string($db,$hokId)."' and p.doelId = 3 and dmafsluit is not null
	GROUP BY p.hokId
 ) endspn on (endspn.hokId = b.hokId)
WHERE b.hokId = '".mysqli_real_escape_string($db,$hokId)."' and ht.datum > coalesce(dmstop,'1973-09-11') 
 and (h.datum > prnt.datum || (h.datum = prnt.datum && h.hisId >= prnt.hisId) ) and h.skip = 0
...
} // Einde else van if(mysqli_num_rows($zoek_doelgroep_3_nu_in_verblijf) > 0 )

} // Einde else van if(mysqli_num_rows($zoek_doelgroep_2_uit_verblijf_na_afsluitdatum) > 0 )

} // Einde else van if(mysqli_num_rows($zoek_doelgroep_2_in_verblijf_voor_en_na_afsluitdatum) > 0 )

} // Einde else van if(mysqli_num_rows($zoek_doelgroep_1_uit_verblijf_na_afsluitdatum) > 0 )

} // Einde else van if(mysqli_num_rows($zoek_doelgroep_1_in_verblijf_voor_en_na_afsluitdatum) > 0 )

	} // Einde while($row = mysqli_fetch_assoc($zoek_verblijven))

$array_hokIds .= 0;
$array_hokIds = str_replace(',0', '', $array_hokIds);

#echo '$array_hokIds = '. $array_hokIds.'<br>';



$zoek_verblijven_in_gebruik = "
SELECT h.hokId, h.hoknr, count(distinct schaap_geb) maxgeb, count(distinct schaap_spn) maxspn, count(distinct schaap_prnt) maxprnt, min(dmin) eerste_in, max(dmuit) laatste_uit
FROM (
	SELECT b.hokId, st.schaapId schaap_geb, NULL schaap_spn, NULL schaap_prnt, h.datum dmin, NULL dmuit
	FROM tblBezet b
	 ...
	 (
		...
		FROM tblBezet b
		 ...
		 join tblActie a2 on (a2.actId = h2.actId)
		 join tblStal st on (h1.stalId = st.stalId)
		WHERE b.hokId in (".mysqli_real_escape_string($db,$array_hokIds).") and a2.uit = 1 and h1.skip = 0 and h2.skip = 0
		GROUP BY b.bezId
	 ) uit on (uit.bezId = b.bezId)
	 left join (
		SELECT st.schaapId, h.datum
	...
	WHERE b.hokId in (".mysqli_real_escape_string($db,$array_hokIds).") and isnull(uit.bezId)
	and isnull(spn.schaapId)
	and isnull(prnt.schaapId)
 	and h.skip = 0
	...
	FROM tblBezet b
	 ...
	 (
		...
		FROM tblBezet b
		 ...
		 join tblActie a2 on (a2.actId = h2.actId)
		 join tblStal st on (h1.stalId = st.stalId)
		WHERE b.hokId in (".mysqli_real_escape_string($db,$array_hokIds).") and a2.uit = 1 and h1.skip = 0 and h2.skip = 0 and h1.actId != 2
		GROUP BY b.bezId
	 ) uit on (uit.bezId = b.bezId)
	 join tblHistorie ht on (ht.hisId = uit.hist)
	 join tblStal st on (st.stalId = h.stalId)
	 ...
	 left join (
		...
		FROM tblPeriode p
		 join tblHok h on (h.hokId = p.hokId)
		WHERE h.hokId in (".mysqli_real_escape_string($db,$array_hokIds).") and p.doelId = 1 and dmafsluit is not null
		GROUP BY p.hokId
	 ) endgeb on (endgeb.hokId = b.hokId)
	WHERE b.hokId in (".mysqli_real_escape_string($db,$array_hokIds).") and ht.datum > coalesce(dmstop,'1973-09-11') 
	 and ( isnull(spn.schaapId)  or (spn.datum  > coalesce(dmstop,'1973-09-11') and 
	 		( h.datum < spn.datum || (h.datum = spn.datum && h.hisId < spn.hisId) ) )
	 	 )
	 and ( isnull(prnt.schaapId) or (prnt.datum > coalesce(dmstop,'1973-09-11') and h.datum < prnt.datum) )
	...
	FROM tblBezet b
	 ...
	 left join (
		...
		FROM tblPeriode p
		 join tblHok h on (h.hokId = p.hokId)
		WHERE h.hokId in (".mysqli_real_escape_string($db,$array_hokIds).") and p.doelId = 2 and dmafsluit is not null
		GROUP BY p.hokId
	 ) endspn on (endspn.hokId = b.hokId)
	WHERE b.hokId in (".mysqli_real_escape_string($db,$array_hokIds /*9-1-2019 weggehaald and (isnull(prnt.schaapId) or prnt.datum > coalesce(dmstop,'1973-09-11')) */).") and ht.datum > coalesce(dmstop,'1973-09-11') 
	 and (h.datum > spn.datum || (h.datum = spn.datum && h.hisId >= spn.hisId) )
	 and (isnull(prnt.schaapId) or h.datum < prnt.datum)
	 and h.skip = 0
	...
	WHERE b.hokId in (".mysqli_real_escape_string($db,$array_hokIds).") and isnull(uit.bezId) and h.skip = 0
	...
	FROM tblBezet b
	 ...
	 left join (
		...
		FROM tblPeriode p
		 join tblHok h on (h.hokId = p.hokId)
		WHERE h.hokId in (".mysqli_real_escape_string($db,$array_hokIds).") and p.doelId = 3 and dmafsluit is not null
		GROUP BY p.hokId
	 ) endspn on (endspn.hokId = b.hokId)
	WHERE b.hokId in (".mysqli_real_escape_string($db,$array_hokIds).") and ht.datum > coalesce(dmstop,'1973-09-11') 
	 and (h.datum > prnt.datum || (h.datum = prnt.datum && h.hisId >= prnt.hisId) ) and h.skip = 0
...
#echo $zoek_verblijven_in_gebruik.'<br>'.'<br>'.'<br>';
$zoek_verblijven_in_gebruik = mysqli_query($db,$zoek_verblijven_in_gebruik) or die (mysqli_error($db));
...
<?php	unset($tot); } // Einde while($row = mysqli_fetch_assoc($zoek_verblijven_in_gebruik)) ?>
</tr>			
</table>
</form>
</TD>
<?php
Include "menu1_new_Bezet.php"; }
?>
