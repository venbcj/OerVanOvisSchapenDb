...
") or die(mysqli_error($db));
    while ($mi = mysqli_fetch_assoc($zoek_laatste_record)) {
        $impId = $mi['Id'];
    }
...
") or die(mysqli_error($db));
        while ($wa = mysqli_fetch_assoc($zoek_worp_aantallen)) {
            $geboren = $wa['geboren'];
            $levend = $wa['levend'];
        }
    ...
    if (isset($doden) && $doden > 0) {
        for ($d = 1; $d <= $doden; $d++) {
            ...
            echo $insert_dood;
            mysqli_query($db, $insert_dood) or die(mysqli_error($db));
            $zoek_laatste_record =  mysqli_query($db, "
SELECT max(Id) Id FROM impAgrident WHERE lidId = " . mysqli_real_escape_string($db, $lidid) . "
") or die(mysqli_error($db));
            while ($lst = mysqli_fetch_assoc($zoek_laatste_record)) {
                $lstId = $lst['Id'];
            }
