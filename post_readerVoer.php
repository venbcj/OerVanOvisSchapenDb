...
foreach ($array as $recId => $id) {
    ...
    foreach ($id as $key => $value) {
     ...
     /*echo $key.'='.$value.' ';*/ $fldHok = $value;
        }
        if ($key == 'kzlVoer' && !empty($value)) {
     /*echo $key.'='.$value.' ';*/ $fldArtId = $value;
        }
        if ($key == 'txtAfslDatum' && !empty($value)) {
            $dag = date_create($value);
            $fldDmAfsluit =  date_format($dag, 'Y-m-d');
...
") or die(mysqli_error($db));
         ...
         // Uitgangspunt : $toedtot is reeds vermenigvuldigd met standaard aantal !!
