...
if (Auth::is_logged_in()) {
    if ($modtech == 1) {
        ...
        if (isset($_POST['knpZoek'])) {
            $worp_van = $_POST['txtWorp_van'];
            $van = date_format(date_create($worp_van), 'Y-m-d');
            $worp_tot = $_POST['txtWorp_tot'];
            $tot = date_format(date_create($worp_tot), 'Y-m-d');
...
<tr>
<td align="right"><input id = "datepicker1" type= text name = "txtWorp_van" size = "8" value = <?php if (isset($worp_van)) {
echo "$worp_van";
        } ?> ></td>
            <td align="left"><input id = "datepicker2" type= text name = "txtWorp_tot" size = "8" value = <?php if (isset($worp_tot)) {
            echo "$worp_tot";
} ?> >
