<?php 
$versie = "3-9-2017"; /* 16-7-2017 gemaakt 	3-9-2017 kg voer te wijzigen*/
...
$versie = '6-3-2020'; /* Rapport ook zichtbaar gemaakt als voer niet wordt gebruikt */
 session_start(); ?>
<html>
<head>
<title>Rapport</title>		
...
$titel = 'Overzicht voertoediening';
$subtitel = '';
Include "header.php"; ?>
...
") or die (mysqli_error($db));
	while($row = mysqli_fetch_array($kzlVoer))
		{
$kzlkey="$row[artId]";
$kzlvalue="$row[naam]";
...
if (empty($_POST['kzlVoer_'])) { $periode_metVoer = " isnull(i.artId)"; }
else { $periode_metVoer = "i.artId = $_POST[kzlVoer_]"; }
...
$aantjaarmaanden = mysqli_query($db," 
...
WHERE h.lidId = ".mysqli_real_escape_string($db,$lidId)." and ".mysqli_real_escape_string($db,$periode_metVoer)." and p.doelId = $_POST[kzlDoel_]
") or die (mysqli_error($db));
...
WHERE eu.lidId = ".mysqli_real_escape_string($db,$lidId)." and ".mysqli_real_escape_string($db,$periode_metVoer)."
...
FROM tblHok h
 ...
 left join tblVoeding v on (p.periId = v.periId)
 left join tblInkoop i on (i.inkId = v.inkId)
WHERE h.lidId = ".mysqli_real_escape_string($db,$lidId)." and ".mysqli_real_escape_string($db,$periode_metVoer)."
GROUP BY h.hoknr
") or die (mysqli_error($db));
...
FROM tblHok ho
 ...
 left join tblInkoop i on (i.inkId = v.inkId)
 left join tblArtikel a on (a.artId = i.artId) 
WHERE ho.lidId = ".mysqli_real_escape_string($db,$lidId)." and p.doelId = $_POST[kzlDoel_] and ".mysqli_real_escape_string($db,$periode_metVoer)." and ".$resJrmnd." and ".$resHok."
...
$eerstePeriode_met_fictieveStartdatum = "SELECT p.hokId, p.doelId, l.dmcreate dmbegin, min(p.dmafsluit) dmeind FROM tblPeriode p join tblHok ho on (p.hokId = ho.hokId) join tblLeden l on (ho.lidId = l.lidId) WHERE ho.lidId = ".mysqli_real_escape_string($db,$lidId)." and p.doelId = $_POST[kzlDoel_] GROUP BY p.hokId, p.doelId";

$artikel_met_voeraantal_PerPeriode = "SELECT v.periId, i.artId, sum(v.nutat) nutat FROM tblVoeding v join tblPeriode p on (v.periId = p.periId) join tblHok ho on (ho.hokId = p.hokId) join tblInkoop i on (v.inkId = i.inkId) WHERE ho.lidId = ".mysqli_real_escape_string($db,$lidId)." and p.doelId = $_POST[kzlDoel_] GROUP BY v.periId, i.artId";

$allePeriodes_met_BeginEnEindDatum_inlc_Voer = "SELECT p2.periId, p2.hokId, p1.doelId, max(p1.dmafsluit) dmbegin, p2.dmafsluit dmeind, v.artId, v.nutat FROM tblPeriode p1 join tblPeriode p2 on (p1.hokId = p2.hokId and p1.doelId = p2.doelId and p1.dmafsluit < p2.dmafsluit) join tblHok ho on (ho.hokId = p1.hokId) left join ( ".mysqli_real_escape_string($db,$artikel_met_voeraantal_PerPeriode).") v on (p2.periId = v.periId) WHERE ho.lidId = ".mysqli_real_escape_string($db,$lidId)." and p1.doelId = $_POST[kzlDoel_] GROUP BY p2.periId, p2.hokId, p1.doelId, p2.dmafsluit, v.nutat";
...
$begin_eind_periode = "
	...
	FROM (
		...
		FROM (
		 	".mysqli_real_escape_string($db,$eerstePeriode_met_fictieveStartdatum)."
		 ) p1
		 join tblPeriode p on (p1.hokId = p.hokId and p1.doelId = p.doelId and p1.dmeind = p.dmafsluit)
		 left join (
		 	".mysqli_real_escape_string($db,$artikel_met_voeraantal_PerPeriode)."
		 ) v on (p.periId = v.periId)

		union

		".mysqli_real_escape_string($db,$allePeriodes_met_BeginEnEindDatum_inlc_Voer)."
	) p
	 join tblHok ho on (ho.hokId = p.hokId)
	 left join tblArtikel i on (i.artId = p.artId)
	WHERE ho.lidId = ".mysqli_real_escape_string($db,$lidId)." and p.doelId = $_POST[kzlDoel_] and ".mysqli_real_escape_string($db,$periode_metVoer)." and date_format(p.dmeind,'%Y%m') = ".mysqli_real_escape_string($db,$jrmnd)."
";
...
<?php
if($_POST['kzlDoel_'] == 1) { $peendm = ' and (his_in.datum < spn.datum or (isnull(spn.schaapId) and isnull(prn.schaapId)) )'; }
if($_POST['kzlDoel_'] == 2) { $peendm = ' and (his_in.datum >= spn.datum and (his_in.datum < prn.datum or isnull(prn.schaapId)) )'; }
if($_POST['kzlDoel_'] == 3) { $peendm = ' and (his_in.datum >= spn.datum)'; }
$periode_totalen = "
...
FROM tblHok ho
 ...
 left join (
 	...
 	WHERE datum <= '".mysqli_real_escape_string($db,$dmbegin)."' and st.lidId = ".mysqli_real_escape_string($db,$lidId)."
 ...
 left join (
	SELECT hisId, '".mysqli_real_escape_string($db,$dmeind)."' datum
 	FROM tblHistorie h
 	 join tblStal st on (h.stalId = st.stalId)
 	WHERE datum >= '".mysqli_real_escape_string($db,$dmeind)."' and st.lidId = ".mysqli_real_escape_string($db,$lidId)."
...
WHERE ho.lidId = ".mysqli_real_escape_string($db,$lidId)." and ho.hokId = ".mysqli_real_escape_string($db,$hokId)." and p.doelId = $_POST[kzlDoel_] and ".mysqli_real_escape_string($db,$periode_metVoer)." 
 and date_format(p.dmeind,'%Y%m') = $jrmnd and his_in.datum < p.dmeind and coalesce(his_uit.datum,CURDATE()) > p.dmbegin ".mysqli_real_escape_string($db,$peendm)."
...
if($_POST['kzlDoel_'] == 1) { $peendm = ' and (his_in.datum < spn.datum or (isnull(spn.schaapId) and isnull(prn.schaapId)) )'; }
if($_POST['kzlDoel_'] == 2) { $peendm = ' and (his_in.datum >= spn.datum or (isnull(spn.schaapId) and prn.schaapId is not null))'; }
...
FROM tblBezet b
 join (
 	...
 	WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and h.datum < '".mysqli_real_escape_string($db,$dmbegin)."'
 	union 
 ...
 left join (
 	...
 	FROM tblHistorie h
 	 join tblStal st on (st.stalId = h.stalId)
 	WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and h.datum > '".mysqli_real_escape_string($db,$dmeind)."'
 	union 
 	SELECT h.hisId, h.stalId, h.datum
 	FROM tblHistorie h
 	 join tblStal st on (st.stalId = h.stalId)
...
WHERE b.hokId = ".mysqli_real_escape_string($db,$hokId) ."
 and his_in.datum < '".mysqli_real_escape_string($db,$dmeind)."' and (isnull(uit.bezId) or his_uit.datum > '".mysqli_real_escape_string($db,$dmbegin)."') and p.doelId = $_POST[kzlDoel_] ".mysqli_real_escape_string($db,$peendm)."
ORDER BY dmin, dmuit
"; ?>
