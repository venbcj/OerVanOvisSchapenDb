...
<?php
        if (!isset($_POST['radVoer_']) || (isset($_POST['radVoer_']) && $_POST['radVoer_'] == 1 )) {
            echo "checked";
        } ?> > Ja
  <input type = radio name = 'radVoer_' value = 0
<?php
            if (isset($_POST['radVoer_']) && $_POST['radVoer_'] == 0) {
                echo "checked";
            } ?> > Nee
</td>
 <td> </td>
...
<th style = "text-align:center;"valign="bottom";width= 80>
<?php
                    if ($metVoer == 'ja') {
                        echo 'voerdatum';
                    } else {
                        echo 'afsluit- / voerdatum';
                    }
?> <hr></th>
...
<td align = "center" valign = "bottom"> <input type = "submit" name = "knpToon_" value = "Toon" style = "font-size:11px;" > <hr> </td>
<td colspan = 2 align="center"; valign="bottom";> Verwijder<br>&nbsp&nbspvoer&nbsp&nbsp periode<hr></td>
</tr>
<?php
                    // $zoek_startdatum zoekt eerste van de maand en het jaar dat een gebruiker is begonnen met het programma
                    ...
                    while ($mld = mysqli_fetch_assoc($begin_eind_periode)) {  // START LOOP $begin_eind_periode
                        $hokId = $mld['hokId'];
                        $hoknr = $mld['hoknr'];
                        $jaarmnd = $mld['jrmnd'];
                        $periId = $mld['periId'];
                        ...
                        $dmeind = $mld['dmeind'];
                        if ($_POST['kzlDoel_'] == 1) {
                            $filterDoel = ' and (his_in.datum < spn.datum or (isnull(spn.schaapId) and isnull(prn.schaapId)) )';
                        }
                        if ($_POST['kzlDoel_'] == 2) {
                            $filterDoel = ' and (his_in.datum >= spn.datum and (his_in.datum < prn.datum or isnull(prn.schaapId)) )';
                        }
                        if ($_POST['kzlDoel_'] == 3) {
                            $filterDoel = ' and (his_in.datum >= spn.datum)';
                        }
                        ...
                        while ($mld = mysqli_fetch_assoc($periode_totalen)) {
                            ...
                            $schaap1dm = $mld['schaap1dm'];
                            if ($dmschaap1 > $dmbegin) {
                                $begindm = $schaap1dm;
                            }
                            ...
                            $schaapenddm = $mld['schaapenddm'];
                            if (!empty($dmschaapend) && $dmschaapend < $dmeind) {
                                $einddm = $schaapenddm;
                            }
...
<?php
                            ...
                            if ($_POST['kzlDoel_'] == 2) {
                                $filterDoel = ' and (his_in.datum >= spn.datum or (isnull(spn.schaapId) and prn.schaapId is not null))';
                            }
?>
<tr align = "center" style = "font-size:15px;">    
 ...
 <td width = 80 > <input type="text" name = <?php echo "txtDatum_$Id"; ?> style="font-size: 11px;" size =8 value = <?php echo $voerdm; ?> > 
<?php 
                            unset($voerdm);
?> 
 </td>
...
<?php
                            if ($voedId > 0) { 
?>
    <input type="text" name = <?php echo "txtKilo_$Id"; ?> style="font-size: 11px; text-align: right; " size =3 value = <?php echo $kilo; ?> > 
<?php
                            } 
?>
 </td>
...
<?php if (!isset($_POST["radSchaap_$Id"]) || (isset($_POST["radSchaap_$Id"]) && $_POST["radSchaap_$Id"] == 0 )) {
echo "checked";
                            } ?> > Excl.
  <input type = radio name = <?php echo "radSchaap_$Id"; ?> value = 1
<?php if (isset($_POST["radSchaap_$Id"]) && $_POST["radSchaap_$Id"] == 1) {
echo "checked";
} ?> > Incl. schapen
<?php
                            } 
...
<?php
                            if ($voedId > 0) { 
?>
    <input type = checkbox name = <?php echo "chbDelVoer_$Id"; ?> value= 1 style = "font-size:11px;" > 
<?php
                            } 
?> 
 </td>
...
?>
<tr class="detailscheider" height = 10><td>    </td></tr>
<?php
                                while ($sch = mysqli_fetch_array($schaap_gegevens)) {
                                    ...
                                    $kg = $sch['kg'];
?> 
<tr class="schaapdetail" align = "center" style = "font-size:15px;"> 
<td></td>
