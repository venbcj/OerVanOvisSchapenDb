...
WHERE st.lidId = ".mysqli_real_escape_string($db,$lidId)." and mdr.schaapId = ".mysqli_real_escape_string($db,$ooiId)." and h.actId = 1
...
FROM tblSchaap s
 join tblStal st on (st.schaapId = s.schaapId)
 join tblHistorie h on (st.stalId = h.stalId)
WHERE s.volwId = ".mysqli_real_escape_string($db,$volwId)." and s.geslacht = 'ooi' and h.actId = 1 and date_format(h.datum,'%m')*1 = ".mysqli_real_escape_string($db,$mnd)." and date_format(h.datum,'%Y') = ".mysqli_real_escape_string($db,$jaar)."
...
FROM tblSchaap s
 join tblStal st on (st.schaapId = s.schaapId)
 join tblHistorie h on (st.stalId = h.stalId)
WHERE s.volwId = ".mysqli_real_escape_string($db,$volwId)." and s.geslacht = 'ooi' and h.actId = 1 and isnull(st.rel_best) and date_format(h.datum,'%m')*1 = ".mysqli_real_escape_string($db,$mnd)." and date_format(h.datum,'%Y') = ".mysqli_real_escape_string($db,$jaar)."
GROUP BY s.schaapId
...
FROM tblSchaap s
 join tblStal st on (st.schaapId = s.schaapId)
 join tblHistorie h on (st.stalId = h.stalId)
WHERE s.volwId = ".mysqli_real_escape_string($db,$volwId)." and s.geslacht = 'ram' and h.actId = 1 and date_format(h.datum,'%m')*1 = ".mysqli_real_escape_string($db,$mnd)." and date_format(h.datum,'%Y') = ".mysqli_real_escape_string($db,$jaar)."
...
FROM tblSchaap s
 join tblStal st on (st.schaapId = s.schaapId)
 join tblHistorie h on (st.stalId = h.stalId)
WHERE s.volwId = ".mysqli_real_escape_string($db,$volwId)." and s.geslacht = 'ram' and h.actId = 1 and isnull(st.rel_best) and date_format(h.datum,'%m')*1 = ".mysqli_real_escape_string($db,$mnd)." and date_format(h.datum,'%Y') = ".mysqli_real_escape_string($db,$jaar)."
GROUP BY s.schaapId
...
FROM tblSchaap s
 join tblStal st on (st.schaapId = s.schaapId)
 join tblHistorie h on (st.stalId = h.stalId)
WHERE s.volwId = ".mysqli_real_escape_string($db,$volwId)." and isnull(s.geslacht) and h.actId = 1 and date_format(h.datum,'%m')*1 = ".mysqli_real_escape_string($db,$mnd)." and date_format(h.datum,'%Y') = ".mysqli_real_escape_string($db,$jaar)."
...
FROM tblSchaap s
 join tblStal st on (st.schaapId = s.schaapId)
 join tblHistorie h on (st.stalId = h.stalId)
WHERE s.volwId = ".mysqli_real_escape_string($db,$volwId)." and isnull(s.geslacht) and h.actId = 1 and isnull(st.rel_best) and date_format(h.datum,'%m')*1 = ".mysqli_real_escape_string($db,$mnd)." and date_format(h.datum,'%Y') = ".mysqli_real_escape_string($db,$jaar)."
GROUP BY s.schaapId
