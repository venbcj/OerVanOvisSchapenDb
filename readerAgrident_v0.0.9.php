...
$taken = array('Worpregistratie', 'Doodgeboren', 'Groepsgeboorte', 'Verplaatsing', 'Spenen', 'Tussenweging', 'Afvoer', 'Aanvoer', 'Omnummeren', 'Medicaties', 'Halsnummers', 'Groepsafvoer', 'Groepsaanvoer', 'Voerregistratie', 'Dekken', 'Dracht', 'StallijstScan', 'StallijstNew');
...
if($i == 12) { $inhoud = $item -> {$taken[$i]}; $velden = array('ActId', 'Datum', 'Ubn', 'HokId', 'Transponder', 'Levensnummer');
												include "impAgrident.php"; }
...
if($i == 15) { $inhoud = $item -> {$taken[$i]}; $velden = array('ActId', 'Datum', 'MoederTransponder', 'Moeder', 'Drachtig', 'Grootte');
												...
												include "impAgrident_dieren.php";*/ 
												include "impAgrident.php"; }
if($i == 16) { $inhoud = $item -> {$taken[$i]}; $velden = array('ActId', 'Datum', 'Transponder', 'Levensnummer');
												include "impAgrident.php"; }

if($i == 17) { $inhoud = $item -> {$taken[$i]}; $velden = array('ActId', 'Datum', 'RasId', 'DoelId', 'HokId', 'Transponder', 'Levensnummer', 'vdrId', 'Geslacht');
												include "impAgrident.php"; }
echo $i.'<br>';
			 } 
