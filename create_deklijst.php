...
for ($i = 1 ; $i <= $weken_jaar ; $i++){
    ...
    $juiste_jaar = date('Y',$day+($i*86400*7)); // Soms valt de eerste maandag in het vorige jaar bijv. 29-12-2014 is week 1 van 2015. 
    if ($juiste_jaar == $year) {
        $Dek_jaar = "INSERT INTO tblDeklijst
            SET lidId = ".mysqli_real_escape_string($db,$lidId).",
            dmdek = '$datum' ";
        mysqli_query($db, $Dek_jaar) or die('QF01'.mysqli_error($db));
    }
}
...
while ($zjr = mysqli_fetch_assoc($zoek_jaar)) {
    $liq_jaar = $zjr['jaar'];
}
if (!isset($liq_jaar)) {
    $new_jaar = $year;
